...
import { ArrowLeft, ArrowRight, RotateCcw, Home, Plus, X, MoreVertical, Star, Settings } from 'lucide-react';
...
const Browser = () => {
  ...
  const handleAddressBarSubmit = (e: React.FormEvent) => {
    ...
    if (url.includes(' ') || (!url.includes('.') && !url.startsWith('chrome://') && !url.startsWith('punsta://'))) {
      ...
      // Add domain if it looks like a domain
      url = url;
    }
  ...
  return (
    <div className="max-w-full mx-auto bg-white shadow-2xl overflow-hidden h-screen flex flex-col">
      {/* Chrome-style tabs */}
      <div className="bg-gray-100 flex items-end border-b border-gray-300">
        <div className="flex items-end flex-1">
          {tabs.map((tab) => (
            <div
              key={tab.id}
              className={`group relative flex items-center px-4 py-2 min-w-0 max-w-64 cursor-pointer border-r border-gray-300 ${
                activeTabId === tab.id 
            ...
            >
              <div className="flex items-center min-w-0 flex-1">
                <div className="w-4 h-4 mr-2 bg-gray-300 rounded-sm flex-shrink-0"></div>
          ...
          <button
            ...
            className="p-2 hover:bg-gray-200 rounded-t"
          >
      ...
      <div className="bg-white px-3 py-2 flex items-center space-x-2 border-b border-gray-200">
        ...
        <form onSubmit={handleAddressBarSubmit} className="flex-1 flex mx-4">
          <div className="flex-1 relative">
            <Input
              ...
              className="w-full pl-8 pr-12 py-1 text-sm border-gray-300 rounded-full bg-gray-50 focus:bg-white"
            ...
            <button className="absolute right-3 top-1/2 transform -translate-y-1/2 hover:bg-gray-200 rounded p-1">
